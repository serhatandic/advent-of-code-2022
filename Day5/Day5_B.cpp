...
void moveTheCrates(vector<vector<char>> refactoredInput, vector<string> input){
    ...
    for (int i = 10; i < input.size(); i++){
        ...
        for (int j = 0; j < howMany; j++){
            refactoredInput[to - 1].insert(refactoredInput[to - 1].begin(), refactoredInput[from - 1][howMany - j - 1]);
            refactoredInput[from - 1].erase(refactoredInput[from - 1].begin() + howMany -j - 1);
        }
    }
